function [reservations,flights] = LEM_requests2reservations(airways,...
    requests,hd)
...
%    waitbar(k/num_requests);
    [k,num_requests];
    [reservations,f1] = LEM_gen_reservations(airways,flights,...
        reservations,requests(k),1,hd);
    indexes = LEM_find_conflict(reservations,0.1);
    if ~isempty(indexes)
