function [model,res] = LEM_traj_analysis_MFI_FAA
%
...
for k = 1:100
    ...
    trajk = traj;
    n = sqrt(rand*2);
    trajk(:,2) = trajk(:,2) + n*randn(length(trajk(:,2)),1);
    trajk(:,3) = trajk(:,3) + n*randn(length(trajk(:,3)),1);
    trajk(:,4) = trajk(:,4) + n*randn(length(trajk(:,4)),1);
