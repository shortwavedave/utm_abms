classdef ATOCUnitTests < matlab.unittest.TestCase
    ...
    methods(Test) % Multiple UAS No Radar Tests - Stress Tests
        ...
        % over a varity of time.
            ...
            for num = 0:numUAS-1
                ...
                vertid = lbsd.getLaneVertexes(res.lane_id);
                pos = lbsd.getVertPositions(vertid);
                uas = ATOCUnitTests.UASSetup(pos, num2str(counter));
                uas.res_ids = res.id;
                s = struct("UAS", uas, "Start", startTime, "End", endTime);
                list_uas = [list_uas; s];
                counter = counter + 1;
    ...
    methods(Test) % Multiple UAS No Radar Tests - Stress Tests
            ...
            while(atoc.time < endTime)
                ...
                for index = 1:length(list_uas)
                    ...
                    elseif (s.Start <= atoc.time)
                        inFlight = inFlight + 1;
                        uas = s.UAS;
                        uas.gps.lon = uas.gps.lon + abs(rand())+1;
                        uas.gps.lat = uas.gps.lat + abs(rand())+1;
                        uas.gps.alt = uas.gps.alt + rand();
                        uas.gps.commit();
                    end
                end
        ...
        % over a varity of time.
            ...
            for num = 0:numUAS-1
                % Randomly pick lanes
                index = randi(length(lane_ids));
                index2 = randi(length(lane_ids));
                while (index == index2)
                    index2 = randi(length(lane_ids));
                end
                ...
                % Create new reservation
                lbsd = ATOCUnitTests.SpecificLBSDReservationSetup(lbsd, ...
                        lane_ids(index), startTime, endTime, ...
                        1, 5, num2str(counter));
                ...
                % Create new reservation
                lbsd = ATOCUnitTests.SpecificLBSDReservationSetup(lbsd, ...
                        lane_ids(index2), startTime, endTime, ...
                        1, 5, num2str(counter));
